...
export class RoomAudioManager {
   ...
   * Requirements: 10.6, 10.7 - Fallback mechanisms for audio initialization failures
   */
  private async handleAudioRecovery(action: RecoveryAction, context: ErrorContext): Promise<void> {
    console.log('🔧 RoomAudioManager: Executing recovery action', action, 'for', context.errorType);
    ...
    try {
      ...
      console.error('❌ RoomAudioManager: Recovery action failed', action, error);
    }
  }
  ...
  private async retryAudioInitialization(): Promise<void> {
    ...
    try {
      ...
      console.error('❌ RoomAudioManager: Audio initialization retry failed', error);
      throw error;
    }
  }
   ...
   */
  async initializeForRoom(roomUsers: RoomUser[], instrumentManager?: any): Promise<void> {
    try {
      console.log("🎵 RoomAudioManager: Initializing audio context for room join");
      ...
      try {
        ...
        if (this.audioContext.state !== "running") {
          throw new Error(`Audio context state is ${this.audioContext.state}, expected running`);
        }
      } catch (error) {
        console.error("❌ RoomAudioManager: Failed to resume audio context:", error);
      ...
      try {
        ...
        console.error("❌ RoomAudioManager: Failed to preload instruments:", error);
        ...
        console.warn("⚠️ RoomAudioManager: Continuing with partial initialization");
      }
      
      console.log("✅ RoomAudioManager: Audio context initialized and instruments preloaded");
    } catch (error) {
      console.error("❌ RoomAudioManager: Failed to initialize audio context:", error);
      
      // Final error report if not already handled
      if (!this.errorRecoveryService.isRecoveryInProgress({
        errorType: ErrorType.AUDIO_INITIALIZATION_FAILED
      })) {
        await this.errorRecoveryService.handleError({
          errorType: ErrorType.AUDIO_INITIALIZATION_FAILED,
   ...
   */
  private async preloadExistingUserInstruments(roomUsers: RoomUser[]): Promise<void> {
    ...
    if (instrumentsToPreload.length === 0) {
      console.log("🎵 RoomAudioManager: No instruments to preload for existing users");
      return;
    }

    console.log(`🎵 RoomAudioManager: Preloading ${instrumentsToPreload.length} instruments for existing users`);
   ...
   */
  async handleUserInstrumentChange(userId: string, username: string, instrument: string, category: string): Promise<void> {
    if (!this.isInitialized) {
      console.warn("🎵 RoomAudioManager: Cannot preload instrument - audio context not initialized");
      return;
    }
    ...
    if (this.preloadedInstruments.has(instrumentKey)) {
      console.log(`🎵 RoomAudioManager: Instrument ${instrument} already preloaded for ${username}`);
      return;
    }

    console.log(`🎵 RoomAudioManager: Preloading new instrument ${instrument} (${category}) for ${username}`);
    ...
    try {
      ...
      console.error(`❌ RoomAudioManager: Failed to preload instrument for ${username}:`, error);
   ...
   */
  private async preloadInstruments(instruments: InstrumentPreloadData[]): Promise<void> {
    if (!this.instrumentManager) {
      console.warn("🎵 RoomAudioManager: No instrument manager available for preloading");
      return;
    }
    ...
    try {
      ...
      const preloadPromises = instruments.map(async (instrument) => {
        ...
        try {
          ...
          console.error(`❌ RoomAudioManager: Failed to preload ${instrument.instrument} for ${instrument.username}:`, error);
      ...
      if (failures > 0) {
        console.warn(`⚠️ RoomAudioManager: ${failures} out of ${instruments.length} instruments failed to preload`);
      }
    }
  }
   ...
   */
  private async handleInstrumentLoadFailure(instrument: InstrumentPreloadData, originalError: any): Promise<void> {
    console.log(`🔄 RoomAudioManager: Attempting fallback for failed instrument ${instrument.instrument}`);
    ...
    try {
      ...
      if (fallbackInstrument) {
        ...
        console.log(`✅ RoomAudioManager: Successfully loaded fallback instrument ${fallbackInstrument} for ${instrument.username}`);
      } else {
        console.warn(`⚠️ RoomAudioManager: No fallback available for ${instrument.instrument} in category ${instrument.category}`);
      ...
      console.error(`❌ RoomAudioManager: Fallback also failed for ${instrument.username}:`, fallbackError);
   ...
   */
  private async findFallbackInstrument(originalInstrument: string, category: InstrumentCategory): Promise<string | null> {
    // Import the fallback utility
    try {
      const { findNextCompatibleInstrument } = await import("@/shared/utils/webkitCompat");
      return await findNextCompatibleInstrument(originalInstrument, category, new Set([originalInstrument]));
    } catch (error) {
  ...
  handleUserLeft(userId: string): void {
    console.log(`🎵 RoomAudioManager: Cleaning up instruments for user ${userId}`);
    ...
    const instrumentsToRemove = Array.from(this.preloadedInstruments)
    ...
    if (user) {
      console.log(`✅ RoomAudioManager: Cleaned up instruments for ${user.username} (${userId})`);
    }
  }
  ...
  private setupUserInteractionHandlers(): void {
    const resumeAudioContext = async () => {
      if (this.audioContext && this.audioContext.state === "suspended") {
        try {
          await this.audioContext.resume();
          console.log("🎵 RoomAudioManager: Audio context resumed after user interaction");
        } catch (error) {
          console.error("❌ RoomAudioManager: Failed to resume audio context:", error);
        }
      }
