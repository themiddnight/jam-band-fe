...
export class ErrorRecoveryService {
  ...
  private recoveryHandlers = new Set<
    (recovery: { action: RecoveryAction; context: ErrorContext }) => void
  >();
  private userFeedbackHandlers = new Set<
    (message: string, type: "error" | "warning" | "info" | "success") => void
  >();
  ...
  async handleError(context: ErrorContext): Promise<void> {
    ...
    if (
      ...
      strategy.action !== RecoveryAction.NO_ACTION
    ) {
    ...
    switch (context.errorType) {
      case ErrorType.NAMESPACE_CONNECTION_FAILED:
        ...
        return {
          ...
          userMessage:
            retryCount > 0
              ? `Retrying connection (${retryCount + 1}/${this.options.maxRetries})...`
              : undefined,
        };
      ...
      case ErrorType.WEBRTC_CONNECTION_FAILED:
        return {
          ...
          userMessage:
            "Voice connection failed. Continuing without voice chat.",
          fallbackStrategy: {
            action: RecoveryAction.NO_ACTION, // Continue without WebRTC
            userMessage:
              "Voice chat unavailable. You can still play music together.",
          },
        };
      ...
      case ErrorType.AUDIO_INITIALIZATION_FAILED:
        return {
          ...
          fallbackStrategy: {
            ...
            userMessage:
              "Audio initialization failed. Please check your browser permissions and try refreshing the page.",
            requiresUserAction: true,
          },
        };
      ...
      case ErrorType.STATE_INCONSISTENCY:
        return {
          ...
          fallbackStrategy: {
            ...
            userMessage:
              "Unable to recover connection state. Returning to lobby.",
            requiresUserAction: true,
          },
        };
      ...
      case ErrorType.GRACE_PERIOD_EXPIRED:
        return {
          ...
          fallbackStrategy: {
            action: RecoveryAction.RETURN_TO_LOBBY,
            userMessage:
              "Unable to reconnect. Please refresh the page or return to lobby.",
            requiresUserAction: true,
          },
        };
      ...
      case ErrorType.PERMISSION_DENIED:
        return {
          action: RecoveryAction.SHOW_USER_PROMPT,
          userMessage:
            "Permission denied. Please check your browser settings and refresh the page.",
          requiresUserAction: true,
        };
   ...
   * Notify recovery handlers
   */
  private notifyRecoveryHandlers(recovery: {
    action: RecoveryAction;
    context: ErrorContext;
  }): void {
