...
export class AdaptiveAudioManager {
  ...
  private onConfigChange: ((config: typeof ADAPTIVE_AUDIO_CONFIG.SMALL_MESH) => void) | null = null;
  ...
  public initialize(audioContext: AudioContext, onConfigChange?: (config: typeof ADAPTIVE_AUDIO_CONFIG.SMALL_MESH) => void) {
  ...
  public updateUserCount(userCount: number) {
    ...
    if (newConfig.quality !== this.state.currentConfig.quality) {
      ...
      this.state.quality = newConfig.quality as 'ultra-low-latency' | 'balanced' | 'stable';
      ...
      // Log the adjustment
      console.log(`🎵 Audio quality adjusted: ${previousConfig.quality} → ${newConfig.quality} (${userCount} users)`);
      console.log(`📊 New config: ${newConfig.description} (target: ${newConfig.latencyTarget})`);
    }
  }
  ...
  private reduceQuality() {
    ...
    if (newConfig.quality !== currentQuality) {
      ...
      console.log(`⚠️ Quality reduced due to performance: ${currentQuality} → ${newConfig.quality}`);
      console.log(`📊 New config: ${newConfig.description} (target: ${newConfig.latencyTarget})`);
    }
  }
  ...
  public getRecommendations() {
    ...
    if (this.state.currentLatency && this.state.currentLatency > parseFloat(this.state.currentConfig.latencyTarget.split('-')[1])) {
      recommendations.push(`Latency (${this.state.currentLatency}ms) exceeds target (${this.state.currentConfig.latencyTarget})`);
    }
    ...
    if (this.state.userCount > 6) {
      recommendations.push(`Large mesh (${this.state.userCount} users) - consider reducing quality for stability`);
    }
