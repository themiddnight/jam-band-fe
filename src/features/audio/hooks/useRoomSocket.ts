...
export const useRoomSocket = (instrumentManager?: any) => {
    ...
    const requestSynthParamsResponseCallbackRef = useRef<
        ...
        | null
    >(null);
    const guestCancelledCallbackRef = useRef<((userId: string) => void) | null>(null);
    const memberRejectedCallbackRef = useRef<((userId: string) => void) | null>(null);
    // Stable ref to call joinRoom from early effects without creating a dep cycle
    const joinRoomRef = useRef<((roomId: string, username: string, userId: string, role: "band_member" | "audience") => void) | null>(null);
    ...
    useEffect(() => {
        ...
        const unsubscribeStateChange = socketManagerRef.current.onStateChange((state, config) => {
        ...
        if (state === ConnectionState.IN_ROOM && config?.roomId && joinedRoomRef.current !== config.roomId) {
                const userId = useUserStore.getState().userId;
                const username = useUserStore.getState().username;
                if (userId && username) {
                    console.log('🚪 Auto-joining room after state change to IN_ROOM:', config.roomId);
                    joinedRoomRef.current = config.roomId;
            // Use ref to avoid effect dependency on joinRoom
            joinRoomRef.current?.(config.roomId, username, userId, config.role || 'audience');
                }
            }
        ...
        const unsubscribeReconnection = socketManagerRef.current.onReconnection(() => {
            ...
            const storedInstrumentState = socketManagerRef.current?.getStoredInstrumentState();
            if (storedInstrumentState && instrumentManager) {
                console.log('🎵 Restoring instrument state:', storedInstrumentState);
    ...
    const safeEmit = useCallback(
        (event: string, data: any) => {
            ...
            if (socket?.connected) {
                ...
                if (event === "play_note" || event === "change_instrument" || event === "join_room" || event === "leave_room" || event === "update_synth_params") {
                    console.log(`📤 Emitting ${event} to backend:`, data);
                    socket.emit(event, data);
                } else {
                    console.log(`📥 Queueing ${event}:`, data);
                    queueMessage(event, data);
                }
            } else {
                console.log(`⏳ Socket not connected, queueing ${event}:`, data);
                pendingOperationsRef.current.push(() => {
                    const currentSocket = getActiveSocket();
                    if (currentSocket?.connected) {
                        console.log(`📤 Emitting queued ${event}:`, data);
    ...
    const throttledEmit = useMemo(
        () => throttle((event: string, data: any) => {
            console.log('🎛️ throttledEmit called with:', event, data);
            safeEmit(event, data);
        }, 10),
        [safeEmit],
    );
    ...
    useEffect(() => {
        ...
        if (connectionState === ConnectionState.LOBBY) {
            ...
            on("room_joined", async (data: { room: any; users: any[]; pendingMembers: any[] }) => {
                setCurrentRoom({
                    ...data.room,
                    users: data.users,
                ...
                if (currentUserId) {
                    const currentUserData = data.users.find((user: any) => user.id === currentUserId);
                    if (currentUserData) {
                ...
                try {
                    console.log("🎵 Initializing audio context and preloading instruments for room join");
                    await roomAudio.initializeForRoom(data.users);
                    console.log("✅ Audio context initialized and instruments preloaded successfully");
                } catch (error) {
            ...
            on("user_joined", async (data: { user: any }) => {
                ...
                if (data.user.currentInstrument && data.user.currentCategory) {
                    try {
                        console.log(`🎵 Preloading instrument for new user: ${data.user.username} - ${data.user.currentInstrument} (${data.user.currentCategory})`);
                        ...
                        console.log(`✅ Successfully preloaded instrument ${data.user.currentInstrument} for new user ${data.user.username}`);
                    } catch (error) {
                        console.error(`❌ Failed to preload instrument for new user ${data.user.username}:`, error);
                    }
                } else {
                    console.log(`⚠️ New user ${data.user.username} has no instrument info:`, data.user);
                }
            });
            ...
            on("member_approved", (data: { user?: any; room?: any }) => {
                ...
                if (data.room) {
                    ...
                    const currentUserId = useUserStore.getState().userId;
                    if (currentUserId) {
                        const approvedUser = data.room.users.find((user: any) => user.id === currentUserId);
                        if (approvedUser) {
            ...
            on("instrument_changed", async (data: {
                userId: string;
                username: string;
                instrument: string;
                category: string;
            }) => {
                console.log("🎵 instrument_changed event received:", data);
                updateUserInstrument(data.userId, data.instrument, data.category);
                // Handle real-time instrument preloading - Requirement 10.3
                try {
                    console.log(`🎵 Preloading instrument via socket handler: ${data.instrument} for ${data.username}`);
                    await roomAudio.handleUserInstrumentChange(
                        data.userId,
                        data.username,
                        data.instrument,
                    ...
                    console.log(`✅ Successfully preloaded via socket handler: ${data.instrument} for ${data.username}`);
                } catch (error) {
                ...
                if (instrumentChangedCallbackRef.current) {
                    console.log("🎵 Calling instrumentChangedCallback with data:", data);
                    instrumentChangedCallbackRef.current(data);
                } else {
                    console.log("⚠️ No instrumentChangedCallback registered");
                }
            });
            ...
            on("request_synth_params_response", (data: { requestingUserId: string; requestingUsername: string }) => {
            ...
            on("approval_request_cancelled", (data: { userId: string; message: string }) => {
    ...
    }, [connectionState, getActiveSocket, setCurrentRoom, setCurrentUser, setError, addUser, removeUser, addPendingMember, removePendingMember, updateUserInstrument, transferOwnership, clearRoom, setPendingApproval, executePendingOperations, setIsConnecting, roomAudio]);
    ...
    const connectToRoom = useCallback(async (roomId: string, role: 'band_member' | 'audience') => {
        if (!socketManagerRef.current) return;
        setIsConnecting(true);
        try {
            ...
            await socketManagerRef.current.connectToRoom(roomId, role, userId || undefined, username || undefined);
        } catch (error) {
    ...
    const requestRoomApproval = useCallback(async (roomId: string, userId: string, username: string, role: 'band_member' | 'audience') => {
        if (!socketManagerRef.current) return;
        setIsConnecting(true);
        try {
            await socketManagerRef.current.connectToApproval(roomId, userId, username, role);
        } catch (error) {
    ...
    const joinRoom = useCallback(
        (roomId: string, username: string, userId: string, role: "band_member" | "audience") => {
            ...
            if (!isUuid) {
                console.warn("join_room aborted: roomId is not a valid UUID v4", { roomId });
                setError("Invalid room identifier. Please return to lobby.");
                return;
            }
            if (!username || !userId) {
                console.warn("join_room aborted: missing username/userId", { username, userId });
            ...
            if (role !== 'band_member' && role !== 'audience') {
                ...
                setError("Invalid role. Please try joining as audience or band member.");
                return;
            }
            console.log("➡️ Emitting join_room", { roomId, username, userId, role });
    ...
    const playNote = useCallback(
        (data: NoteData) => {
            ...
            if (!Array.isArray(data.notes)) {
                console.error('playNote received non-array notes:', data.notes, 'Full data:', data);
                return;
            }
    ...
    const updateSynthParams = useCallback(
        (params: Partial<SynthState>) => {
            ...
            console.log('🎛️ About to call throttledEmit with update_synth_params');
    ...
    const onNoteReceived = useCallback(
        (callback: (data: NoteReceivedData) => void) => {
            const socket = getActiveSocket();
            if (!socket) {
                console.log("⚠️ No active socket for onNoteReceived");
                return () => { };
            }
            ...
            const handleNoteReceived = (data: NoteReceivedData) => {
                console.log("🎵 Raw note_played event received:", data);
                callback(data);
            };
            ...
            return () => {
                console.log("🧹 Removing note_played listener");
    ...
    const onInstrumentChanged = useCallback(
        (callback: (data: {
            ...
            const socket = getActiveSocket();
            if (!socket) {
                console.log("⚠️ No active socket for onInstrumentChanged");
                return () => { };
            }

            console.log("🔧 Setting up instrument_changed listener on socket:", socket.id);
