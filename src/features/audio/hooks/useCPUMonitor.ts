  ...
  const getRecommendations = useCallback(() => {
    ...
    if (cpuUsage && cpuUsage > 90) {
      recommendations.push(
        "CPU usage is very high - consider closing other applications",
      );
      ...
      recommendations.push(
        "CPU usage is moderate - monitor for performance issues",
      );
    }
    ...
    if (trend === "increasing") {
      recommendations.push(
        "CPU usage is trending upward - consider reducing audio quality",
      );
    }
