...
export interface UseRoomAudioReturn {
  ...
  handleUserInstrumentChange: (userId: string, username: string, instrument: string, category: string) => Promise<void>;
 ...
 */
export const useRoomAudio = ({ connectionState, instrumentManager }: UseRoomAudioOptions): UseRoomAudioReturn => {
  ...
  useEffect(() => {
    if (connectionState !== ConnectionState.IN_ROOM && roomAudioManagerRef.current) {
      roomAudioManagerRef.current.cleanup();
    }
  }, [connectionState]);

  // Initialize audio for room - Requirements 10.1, 10.2
  const initializeForRoom = useCallback(async (roomUsers: RoomUser[]) => {
    if (!roomAudioManagerRef.current) {
    ...
    try {
      await roomAudioManagerRef.current.initializeForRoom(roomUsers, instrumentManager);
    } catch (error) {
  ...
  const handleUserInstrumentChange = useCallback(async (
    userId: string,
    username: string,
    instrument: string,
    category: string
    ...
    if (!roomAudioManagerRef.current) {
      console.warn("⚠️ RoomAudioManager not initialized for instrument change");
      return;
    }

    try {
      await roomAudioManagerRef.current.handleUserInstrumentChange(userId, username, instrument, category);
    } catch (error) {
