...
export const AudioEditor = ({ region }: AudioEditorProps) => {
  ...
  return (
    <div className="flex flex-col h-full bg-base-100">
      ...
      {/* Info bar */}
      <div className="flex items-center gap-4 px-4 py-2 bg-base-200 text-xs text-base-content/70">
        <div>
          <span className="font-semibold">Duration:</span>{' '}
          {region.length.toFixed(2)} beats
        </div>
        <div>
          <span className="font-semibold">Trim Start:</span>{' '}
          {(region.trimStart || 0).toFixed(2)} beats
        </div>
        <div>
          <span className="font-semibold">Original:</span>{' '}
          {(region.originalLength || region.length).toFixed(2)} beats
        </div>
      </div>
