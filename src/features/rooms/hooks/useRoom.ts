...
export const useRoom = () => {
  ...
  const [fallbackNotification, setFallbackNotification] = useState<{ message: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (connectionState !== ConnectionState.IN_ROOM && connectionState !== ConnectionState.REQUESTING) {
      connectToRoom(roomId, role);
    }
  }, [roomId, username, userId, location.state, connectionState, connectToRoom, navigate]);
  ...
  useEffect(() => {
    ...
    if (connectionState !== ConnectionState.IN_ROOM) {
      console.log('🔧 Skipping onNoteReceived setup - not in room yet:', connectionState);
      return;
    }
    
    console.log('🔧 Setting up onNoteReceived handler - connection state:', connectionState);
    const unsubscribe = onNoteReceived(async (data) => {
      ...
      if (data.eventType === "note_on") {
        setPlayingIndicators(prev => {
          ...
          newMap.set(data.userId, { velocity: data.velocity, timestamp: Date.now() });
          return newMap;
        });
        ...
        try {
          console.log('🎵 Playing remote notes:', data.notes, 'from user:', data.userId, 'instrument:', data.instrument, 'category:', data.category, 'isKeyHeld:', data.isKeyHeld);
        ...
        // Stop remote notes
        try {
          console.log('🛑 Stopping remote notes:', data.notes, 'from user:', data.userId);
          if (data.notes && data.notes.length > 0) {
        ...
        try {
          setRemoteUserSustain(data.userId, true, data.instrument, data.category as InstrumentCategory);
        } catch (error) {
        ...
        try {
          setRemoteUserSustain(data.userId, false, data.instrument, data.category as InstrumentCategory);
        } catch (error) {
  ...
  const handleInstrumentChanged = useCallback(async (data: any) => {
    ...
    try {
      console.log(`🎵 Updating remote instrument ${data.instrument} (${data.category}) for user ${data.username}`);
      ...
      console.log(`✅ Successfully updated remote instrument ${data.instrument} for user ${data.username}`);
    } catch (error) {
      console.error(`❌ Failed to update remote instrument for ${data.username}:`, error);
  ...
  useEffect(() => {
    // Only set up the handler when we're connected to a room
    if (connectionState !== ConnectionState.IN_ROOM) {
      console.log('🔧 Skipping onInstrumentChanged setup - not in room yet:', connectionState);
      return;
    }
  ...
  const handlePlayNote = useCallback(async (notes: string[], velocity: number, eventType: "note_on" | "note_off" | "sustain_on" | "sustain_off", isKeyHeld?: boolean) => {
    console.log('🎵 handlePlayNote called:', { notes, velocity, eventType, isKeyHeld, isConnected });
    ...
    playNote(noteData);
  }, [isConnected, currentInstrument, currentCategory, playNote, playLocalNote, stopLocalNotes]);

  const handleStopNote = useCallback((notes: string[] | string) => {
    const notesArray = Array.isArray(notes) ? notes : [notes];
    handlePlayNote(notesArray, 0, "note_off");
  }, [handlePlayNote]);

  const handleReleaseKeyHeldNote = useCallback((note: string) => {
    handlePlayNote([note], 0, "note_off", false);
  }, [handlePlayNote]);

  const handleSustainChange = useCallback((sustained: boolean) => {
  ...
  const handleSustainToggleChange = useCallback((sustained: boolean) => {
  ...
  const handleApproveMember = useCallback((userId: string) => {
    approveMember(userId);
  }, [approveMember]);

  const handleRejectMember = useCallback((userId: string) => {
    rejectMember(userId);
  }, [rejectMember]);
  ...
  const handleLeaveRoomConfirm = useCallback(async () => {
    ...
    clearRoom();
    navigate("/");
  }, [connectionState, cancelApprovalRequest, leaveRoom, clearRoom, navigate]);
  ...
  const handleInstrumentChangeWrapper = useCallback(async (instrument: string) => {
    console.log('🎵 Instrument change wrapper called:', instrument, 'category:', currentCategory);
    await handleInstrumentChange(instrument);
    if (isConnected) {
      console.log('🎵 Sending instrument change to remote users:', instrument, currentCategory);
      changeInstrument(instrument, currentCategory);
      
      // Send current synth parameters as preset for the new instrument
      console.log('🎛️ Sending current synth preset for new instrument:', instrument);
  ...
  }, [handleInstrumentChange, isConnected, changeInstrument, currentCategory, synthState, updateSynthParams]);

  const handleCategoryChangeWrapper = useCallback((category: string) => {
    ...
    console.log('🎵 Category changed locally, waiting for instrument change to sync to remote');
  }, [handleCategoryChange]);

  // Synth parameter update
  const updateSynthParamsWrapper = useCallback((params: any) => {
    ...
    if (isConnected) {
      console.log('🎛️ Connected - sending synth params to remote users:', params);
