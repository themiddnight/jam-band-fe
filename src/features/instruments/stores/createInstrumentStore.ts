        ...
        ({
          ...
          setVelocity: (velocity: number) =>
            set({
              velocity: Math.max(
                bounds.velocity.min,
                Math.min(bounds.velocity.max, velocity),
              ),
          ...
          setCurrentOctave: (octave: number) =>
            set({
              currentOctave: Math.max(
                bounds.octave.min,
                Math.min(bounds.octave.max, octave),
              ),
            } as Partial<FullState>),
          setSustain: (sustain: boolean) =>
            set({ sustain } as Partial<FullState>),
          setSustainToggle: (sustainToggle: boolean) =>
            set({ sustainToggle } as Partial<FullState>),

          // Common toggle actions
          toggleSustain: () =>
            set(
              (state: FullState) =>
                ({ sustain: !state.sustain }) as Partial<FullState>,
            ),
          toggleSustainToggle: () =>
            set(
              (state: FullState) =>
                ({ sustainToggle: !state.sustainToggle }) as Partial<FullState>,
            ),
            ...
            set(
              (state: FullState) =>
                ({
                  velocity: Math.min(
                    bounds.velocity.max,
                    state.velocity + bounds.velocity.step,
                  ),
                }) as Partial<FullState>,
            ),
          decrementVelocity: () =>
