...
// Default drum pad assignments for different drum machines
...
export const createFallbackAssignments = (availableSamples: string[]): Record<string, string> => {
...
export const createSmartAssignments = (availableSamples: string[]): Record<string, string> => {
  const assignments: Record<string, string> = {};
  ...
  const unusedSamples = availableSamples.filter(sample => !usedSamples.has(sample));
...
export const validatePresetAssignments = (
  ...
  Object.entries(preset.padAssignments).forEach(([padId, sampleName]) => {
    ...
    if (availableSamples.includes(sampleName)) {
      ...
      const similarSample = availableSamples.find(sample => 
...
export const DRUMPAD_SHORTCUTS = {
  ...
  'pad-0': 'q', 'pad-1': 'w', 'pad-2': 'e', 'pad-3': 'r',
  'pad-4': 'a', 'pad-5': 's', 'pad-6': 'd', 'pad-7': 'f',
  // Group B: uiop jkl; (8 pads)
  'pad-8': 'u', 'pad-9': 'i', 'pad-10': 'o', 'pad-11': 'p',
  'pad-12': 'j', 'pad-13': 'k', 'pad-14': 'l', 'pad-15': ';',
} as const;

// Fixed colors for consistent appearance
export const DRUMPAD_COLORS = [
  // Group A colors (warmer tones)
  "bg-red-500", "bg-orange-500", "bg-yellow-500", "bg-amber-500",
  "bg-pink-500", "bg-rose-500", "bg-red-400", "bg-orange-400",
  // Group B colors (cooler tones)
  "bg-blue-500", "bg-indigo-500", "bg-purple-500", "bg-violet-500",
  "bg-cyan-500", "bg-teal-500", "bg-blue-400", "bg-indigo-400",
] as const; 