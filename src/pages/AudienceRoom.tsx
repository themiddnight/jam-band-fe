...
const AudienceRoom = memo(() => {
  ...
  const [streamStatus, setStreamStatus] = useState<
    "connecting" | "buffering" | "playing" | "error"
  >("connecting");
      ...
      <div className="min-h-dvh bg-base-200 flex items-center justify-center">
        <div className="card bg-base-100 shadow-xl w-full max-w-md">
          <div className="card-body text-center">
            ...
            <p className="text-base-content/70 mb-4">Joining as audience...</p>
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-base-200">
      <div className="flex flex-col items-center p-3 w-full max-w-6xl mx-auto flex-1">
        {/* Room Header */}
          ...
          <div className="flex justify-between items-center gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center">
                <span className="text-sm mr-2">{currentUser?.username}</span>
                <span className="text-sm text-base-content/50">Audience</span>
              </div>
            </div>
            <button onClick={handleLeave} className="btn btn-outline btn-xs">
              Leave Room
            </button>
          </div>
        </div>
        ...
        <div className="w-full  mb-4">
          <div className="card bg-linear-to-r from-primary/20 to-secondary/20 shadow-xl">
            <div className="card-body items-center text-center py-8">
              ...
              {/* Status */}
              <div
                className={`flex items-center gap-2 text-sm mt-2 ${streamStatus === "playing"
                    ? "text-success"
                    : streamStatus === "error"
                      ? "text-error"
                      : "text-warning"
              ...
              >
                <span
                  className={`w-2 h-2 rounded-full ${streamStatus === "playing"
        ...
        <div className="w-full h-full flex flex-col-reverse lg:flex-row gap-3 flex-1">
          {/* Users List Panel */}
          <div className="flex-3 h-vh">
              ...
              <div className="card-body p-4">
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    ...
                    {!currentRoom?.users?.some(
                      (u) =>
                        u.role === "room_owner" || u.role === "band_member"
                    ) && (
                        <p className="text-sm text-base-content/50 italic p-2">
                          No musicians
                        </p>
                      )}
                  </div>
                ...
                <div>
                  <h4 className="text-sm font-semibold text-base-content/70 mb-2 flex items-center gap-2">
                    ...
                    Audience (
                    {currentRoom?.users?.filter((u) => u.role === "audience")
                      .length || 0}
                    )
                  </h4>
                  <div className="flex gap-2 space-y-1 max-h-40 overflow-y-auto">
                    {currentRoom?.users
                      ...
                      .map((user) => (
                        <div
                          key={user.id}
                          className={`w-fit flex items-center gap-2 p-2 rounded-lg ${user.id === currentUser?.id
