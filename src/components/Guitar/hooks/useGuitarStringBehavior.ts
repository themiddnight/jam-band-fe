...
export const useGuitarStringBehavior = (
  ...
  const isHammerOnValid = useCallback(
    (stringId: "lower" | "higher", note: string): boolean => {
      ...
      if (
        ...
        stateRef.current.hammerOnState.windowMs
      ) {
        return false;
      }
  ...
  // Function to handle note press with string behavior
  const handleNotePress = useCallback(
    (stringId: "lower" | "higher", note: string) => {
  ...
  const handlePlayButtonPress = useCallback(
    (stringId: "lower" | "higher", customVelocity?: number) => {
      const currentTime = Date.now();
      ...
      setStrings((prevStrings) => {
        ...
        if (highestNote) {
          ...
          const shouldPlay =
            string.lastPlayedNote !== highestNote || timeSinceLastPlay > 50; // 50ms debounce
  ...
  const handleHammerOnPress = useCallback(
    (stringId: "lower" | "higher", note: string) => {
      const currentTime = Date.now();
