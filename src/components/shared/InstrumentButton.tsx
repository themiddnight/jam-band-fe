...
const getBackgroundColor = (note: string | undefined, variant: 'note' | 'chord' | 'modifier', modifierType?: string): string => {
  if (variant === 'chord') {
...
const getPressedBackgroundColor = (note: string | undefined, variant: 'note' | 'chord' | 'modifier', modifierType?: string): string => {
  if (variant === 'chord') {
...
const getTextColor = (variant: 'note' | 'chord' | 'modifier', modifierType?: string): string => {
  switch (variant) {
    case 'chord':
...
export const InstrumentButton = memo<InstrumentButtonProps>(({
  ...
  const pressedBgColor = getPressedBackgroundColor(note, variant, modifierType);
  ...
  return (
    ...
    >
      ...
      {variant === 'chord' ? (
        ...
        note && (
          <span className={`text-xs ${textColor}`}>{note}</span>
        )
      )}
    </button>
  );
