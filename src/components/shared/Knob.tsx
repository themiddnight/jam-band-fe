...
export const Knob: React.FC<KnobProps> = ({
  ...
  const rotationAngle = orientation === 'vertical' 
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    const newPercentage = Math.max(0, Math.min(1, valueToPercentage(startValue) + deltaPercentage));
  ...
  }, [isDragging, disabled, startY, startValue, orientation, valueToPercentage, percentageToValue, onChange, step, min, max]);
  ...
  const handleTouchMove = useCallback((e: TouchEvent) => {
    ...
    const touch = e.touches[0];
    const currentY = orientation === 'vertical' ? touch.clientY : touch.clientX;
    const delta = startY - currentY;
    const sensitivity = 0.005; // Adjust for sensitivity
    const deltaPercentage = delta * sensitivity;
  ...
  return (
    ...
    >
      <div
        ref={knobRef}
        className={`absolute inset-0 rounded-full border-2 ${getColorClasses('border')} bg-neutral cursor-pointer transition-transform duration-100 ${
          disabled ? 'opacity-50 cursor-not-allowed' : ''
        }`}
        style={knobStyle}
      >
        {/* Knob indicator */}
        <div className={`absolute top-1 left-1/2 w-1 h-3 ${getColorClasses('bg')} rounded-full transform -translate-x-1/2`} />
      </div>
      
      {/* Knob center dot */}
      <div className={`absolute top-1/2 left-1/2 w-2 h-2 ${getColorClasses('bg')} rounded-full transform -translate-x-1/2 -translate-y-1/2`} />
