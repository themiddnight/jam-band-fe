...
import { BRUSHING_TIMES, type BrushingTime } from '../constants/guitarShortcuts';
  ...
  persist(
    (set) => ({
      ...
      toggleMode: () => set((state) => {
        const modes: ('basic' | 'melody' | 'chord')[] = ['basic', 'melody', 'chord'];
      ...
      incrementBrushingSpeed: () => set((state) => {
      ...
      decrementBrushingSpeed: () => set((state) => {
        const speeds = Object.values(BRUSHING_TIMES);
        const currentIndex = speeds.indexOf(state.brushingSpeed);
