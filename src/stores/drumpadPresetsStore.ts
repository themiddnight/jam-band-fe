...
interface DrumpadPresetsState {
  ...
  createSmartDefaultPreset: (instrument: string, availableSamples: string[]) => DrumPreset;
  ...
  persist(
    (set, get) => ({
      ...
      deletePreset: (presetId: string) => {
        ...
        const currentMachinePresets = presets[currentInstrument] || [];
        const filteredPresets = currentMachinePresets.filter(p => p.id !== presetId);
        ...
        const newCurrentPreset = currentPreset?.id === presetId ? null : currentPreset;
